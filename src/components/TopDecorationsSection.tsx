...
interface TopDecorationsSectionProps {
...
const TopDecorationsSection: React.FC<TopDecorationsSectionProps> = ({
  sectionNumber,
  category,
  subcategories,
  title,
  ...
  const { canFetch, isRefreshed, queueRequest } = useFetchControl({ 
    ...
    componentId: `top-decorations-${sectionNumber}`
  }); // 30 seconds delay
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (subcategories && subcategories.length > 0) {
          // Try to fetch from specific subcategories first using queued request
          try {
            categoryEvents = await queueRequest(() => getExperiencesBySubcategories(subcategories));
          } catch (subError) {
          ...
          if (categoryEvents.length === 0) {
            categoryEvents = await queueRequest(() => getEventsByCategory(category));
          }
        } else {
          // Fetch from general category using queued request
          categoryEvents = await queueRequest(() => getEventsByCategory(category));
        }
        
        setEvents(categoryEvents.slice(0, 10)); // Limit to 10 events
        setError(null);
      } catch (err) {
    ...
    fetchEvents();
  }, [canFetch, category, subcategories]);
...
export default TopDecorationsSection;
